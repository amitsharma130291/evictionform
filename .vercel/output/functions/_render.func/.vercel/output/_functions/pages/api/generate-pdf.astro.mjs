...
import { g as getState } from '../../chunks/states_CdPOmSL_.mjs';
...
const POST = async ({ request }) => {
  try {
    ...
    const noticeInfo = stateInfo?.notices[noticeType];
    const chunks = [];
    ...
    await new Promise((resolve) => {
      doc.on("end", resolve);
      doc.fontSize(12).font("Helvetica-Bold").text(`STATE OF ${stateInfo?.name.toUpperCase() || state}`, { align: "center" });
      doc.moveDown(0.5);
      const titles = {
      ...
      doc.fontSize(14).font("Helvetica-Bold").text(titles[noticeType] || "EVICTION NOTICE", { align: "center", underline: true });
      doc.moveDown(1);
      ...
      for (let i = 0; i < pages.count; i++) {
        ...
        doc.fontSize(8).fillColor("#9CA3AF").text(
          "Generated by EvictionForm.com — For informational purposes only. Not legal advice.",
          72,
          doc.page.height - 40,
